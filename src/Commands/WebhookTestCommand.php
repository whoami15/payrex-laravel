...
final class WebhookTestCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $type = $this->argument('type') !== null
            ...
            : search(
                label: 'Search for an event type to simulate',
                placeholder: 'E.g. payment_intent.succeeded',
                options: function (string $value) use ($valid): array {
                    if ($value === '') {
                        return $valid;
                    }

                    return array_values(
                        array_filter($valid, fn (string $v) => Str::contains($v, $value, ignoreCase: true))
                    );
                },
            );
