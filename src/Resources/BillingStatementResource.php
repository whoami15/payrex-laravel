...
final class BillingStatementResource extends ApiResource
{
    ...
    {
        return BillingStatement::from($this->client->post($this->resourceUri(), $this->withDefaultCurrency($params), $this->idempotencyHeader($idempotencyKey)));
    }
    ...
    {
        return BillingStatement::from($this->client->post("{$this->resourceUri()}/{$id}/finalize", [], $this->idempotencyHeader($idempotencyKey)));
    }
    ...
    {
        return BillingStatement::from($this->client->post("{$this->resourceUri()}/{$id}/void", [], $this->idempotencyHeader($idempotencyKey)));
    }
    ...
    {
        return BillingStatement::from($this->client->post("{$this->resourceUri()}/{$id}/mark_uncollectible", [], $this->idempotencyHeader($idempotencyKey)));
    }
    ...
    {
        // return BillingStatement::from($this->client->post("{$this->resourceUri()}/{$id}/send", [], $this->idempotencyHeader($idempotencyKey)));

        $response = $this->client->post("{$this->resourceUri()}/{$id}/send", [], $this->idempotencyHeader($idempotencyKey));
