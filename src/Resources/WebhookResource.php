...
final class WebhookResource extends ApiResource
{
    ...
    {
        return WebhookEndpoint::from($this->client->post($this->resourceUri(), $params, $this->idempotencyHeader($idempotencyKey)));
    }
    ...
    {
        return WebhookEndpoint::from($this->client->get("{$this->resourceUri()}/{$id}"));
    }
    ...
    {
        return WebhookEndpoint::from($this->client->put("{$this->resourceUri()}/{$id}", $params));
    }
    ...
    {
        return WebhookEndpoint::from($this->client->post("{$this->resourceUri()}/{$id}/enable", [], $this->idempotencyHeader($idempotencyKey)));
    }
    ...
    {
        return WebhookEndpoint::from($this->client->post("{$this->resourceUri()}/{$id}/disable", [], $this->idempotencyHeader($idempotencyKey)));
    }
}
