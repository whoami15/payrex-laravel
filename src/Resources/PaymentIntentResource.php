...
final class PaymentIntentResource extends ApiResource
{
    ...
    {
        return PaymentIntent::from($this->transport->request('POST', $this->resourceUri(), $this->withDefaultCurrency($params), $this->idempotencyHeader($idempotencyKey)));
    }
    ...
    {
        return PaymentIntent::from($this->transport->request('GET', "{$this->resourceUri()}/{$id}"));
    }
    ...
    {
        return PaymentIntent::from($this->transport->request('POST', "{$this->resourceUri()}/{$id}/cancel", [], $this->idempotencyHeader($idempotencyKey)));
    }
    ...
    {
        return PaymentIntent::from($this->transport->request('POST', "{$this->resourceUri()}/{$id}/capture", $params, $this->idempotencyHeader($idempotencyKey)));
    }
}
