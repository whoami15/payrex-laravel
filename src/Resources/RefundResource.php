...
final class RefundResource extends ApiResource
{
    ...
    {
        return Refund::from($this->transport->request('POST', $this->resourceUri(), $this->withDefaultCurrency($params), $this->idempotencyHeader($idempotencyKey)));
    }
    ...
    {
        return Refund::from($this->transport->request('PUT', "{$this->resourceUri()}/{$id}", $params));
    }
}
