...
it('creates a customer in PayRex and stores the ID on the model', function () {
    ...
    expect($customer)->toBeInstanceOf(Customer::class)
        ->and($customer->id)->toBe('cus_BbXnRnF9r577BBFFuNsDFvXvBes1Aee1')
        ->and($user->fresh()->payrex_customer_id)->toBe('cus_BbXnRnF9r577BBFFuNsDFvXvBes1Aee1');
});
...
it('retrieves the PayRex customer for the model', function () {
    Http::fake([
        'https://api.payrexhq.com/customers/cus_BbXnRnF9r577BBFFuNsDFvXvBes1Aee1' => Http::response(loadFixture('customer/created.json')),
    ]);

    $user = UserWithPayrexCustomer::create([
        'name' => 'Juan Dela Cruz',
        'email' => 'juan@gmail.com',
        'payrex_customer_id' => 'cus_BbXnRnF9r577BBFFuNsDFvXvBes1Aee1',
    ]);
    ...
    Http::assertSent(function ($request) {
        return $request->url() === 'https://api.payrexhq.com/customers/cus_BbXnRnF9r577BBFFuNsDFvXvBes1Aee1'
            && $request->method() === 'GET';
    });
});
...
it('updates the PayRex customer for the model', function () {
    Http::fake([
        'https://api.payrexhq.com/customers/cus_BbXnRnF9r577BBFFuNsDFvXvBes1Aee1' => Http::response(loadFixture('customer/updated.json')),
    ]);
...
it('deletes the PayRex customer and clears the ID on the model', function () {
    Http::fake([
        'https://api.payrexhq.com/customers/cus_BbXnRnF9r577BBFFuNsDFvXvBes1Aee1' => Http::response(loadFixture('customer/deleted.json')),
    ]);
...
it('returns the PayRex customer ID from the model', function () {
    ...
    expect($user->payrexCustomerId())->toBe('cus_BbXnRnF9r577BBFFuNsDFvXvBes1Aee1')
...
it('supports a custom column name for the PayRex customer ID', function () {
    ...
    expect($merchant->payrexCustomerIdColumn())->toBe('prx_id')
        ->and($merchant->fresh()->prx_id)->toBe('cus_BbXnRnF9r577BBFFuNsDFvXvBes1Aee1');
});
