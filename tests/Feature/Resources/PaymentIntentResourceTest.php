...
it('creates a payment intent', function () {
    ...
    expect($result)->toBeInstanceOf(PaymentIntent::class)
        ...
        ->and($result->clientSecret)->toBe('pi_xxxxx_secret_xxxxx')
...
it('retrieves a payment intent', function () {
    Http::fake(['https://api.payrexhq.com/payment_intents/pi_xxxxx' => Http::response(loadFixture('payment_intent/retrieved.json'))]);

    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');
    $result = $client->paymentIntents()->retrieve('pi_xxxxx');
    ...
    expect($result)->toBeInstanceOf(PaymentIntent::class)
        ...
        ->and($result['latest_payment']['id'])->toBe('pay_xxxxx')
        ->and($result->paymentMethodId)->toBe('pm_xxxxx')
        ->and($result->returnUrl)->toBe('https://my-application.com/return')
        ->and($result->nextAction)->toBeNull()
        ->and($result->customer)->toBeNull();

    Http::assertSent(fn ($r) => $r->url() === 'https://api.payrexhq.com/payment_intents/pi_xxxxx'
        && $r->method() === 'GET'
    );
});

it('cancels a payment intent', function () {
    Http::fake(['https://api.payrexhq.com/payment_intents/pi_xxxxx/cancel' => Http::response(loadFixture('payment_intent/cancelled.json'))]);

    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');
    $result = $client->paymentIntents()->cancel('pi_xxxxx');
    ...
    Http::assertSent(fn ($r) => $r->url() === 'https://api.payrexhq.com/payment_intents/pi_xxxxx/cancel'
        && $r->method() === 'POST'
    );
});

it('captures a payment intent', function () {
    Http::fake(['https://api.payrexhq.com/payment_intents/pi_xxxxx/capture' => Http::response(loadFixture('payment_intent/captured.json'))]);

    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');
    $result = $client->paymentIntents()->capture('pi_xxxxx', ['amount' => 10000]);
    ...
    Http::assertSent(function ($r) {
        return $r->url() === 'https://api.payrexhq.com/payment_intents/pi_xxxxx/capture'
            && $r->method() === 'POST'
            && $r['amount'] === 10000;
    });
});
