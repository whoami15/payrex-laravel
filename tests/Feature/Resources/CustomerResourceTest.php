...
it('lists customers', function () {
    ...
    expect($result)->toBeInstanceOf(PayrexCollection::class)
        ...
        ->and($result->data[0]->id)->toBe('cus_xxxxx')
        ->and($result->data[0]->name)->toBe('Juan Dela Cruz')
        ->and($result->data[1]->id)->toBe('cus_yyyyy')
...
it('retrieves a customer', function () {
    Http::fake(['https://api.payrexhq.com/customers/cus_xxxxx' => Http::response(loadFixture('customer/created.json'))]);

    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');
    $result = $client->customers()->retrieve('cus_xxxxx');
    ...
    Http::assertSent(fn ($r) => $r->url() === 'https://api.payrexhq.com/customers/cus_xxxxx'
        && $r->method() === 'GET'
    );
});

it('updates a customer', function () {
    Http::fake(['https://api.payrexhq.com/customers/cus_xxxxx' => Http::response(loadFixture('customer/updated.json'))]);

    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');
    $result = $client->customers()->update('cus_xxxxx', [
    ...
    Http::assertSent(function ($r) {
        return $r->url() === 'https://api.payrexhq.com/customers/cus_xxxxx'
            && $r->method() === 'PUT'
...
it('deletes a customer', function () {
    Http::fake(['https://api.payrexhq.com/customers/cus_xxxxx' => Http::response(loadFixture('customer/deleted.json'))]);

    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');
    $result = $client->customers()->delete('cus_xxxxx');
