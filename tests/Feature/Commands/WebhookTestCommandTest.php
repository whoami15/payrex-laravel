...
it('returns all event types when search input is empty', function () {
    Event::fake();

    $allTypes = array_column(WebhookEventType::cases(), 'value');

    $this->artisan('payrex:webhook-test')
        ->expectsSearch(
            question: 'Search for an event type to simulate',
            answer: 'payment_intent.succeeded',
            search: '',
            answers: $allTypes,
        )
        ->assertSuccessful()
...
it('prompts for event type when no argument is given', function () {
    Event::fake();
    ...
    $this->artisan('payrex:webhook-test')
        ->expectsSearch(
            question: 'Search for an event type to simulate',
            answer: 'payment_intent.succeeded',
            search: 'payment_intent.succeeded',
            answers: ['payment_intent.succeeded'],
        )
        ->assertSuccessful()
        ->expectsOutputToContain('Dispatched payment_intent.succeeded event successfully.');
