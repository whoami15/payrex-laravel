...
it('returns event data as a typed DTO via data()', function () {
    $payload = loadFixture('webhooks/billing_statement_line_item.deleted.json');
    $event = new BillingStatementLineItemDeleted($payload);
    ...
    expect($data)
        ...
        ->and($data->id)->toBe('bstm_li_vAcSxynQuUoQXkbbDPCcFsVyzKM2Cm3B')
        ->and($data->description)->toBe('Line Item 1')
        ->and($data->unitPrice)->toBe(50000)
        ->and($data->quantity)->toBe(1)
        ->and($data->billingStatementId)->toBe('bstm_vFKeodC5H2mBunNpv6SekjGkuVHq3jGy');
});
...
it('returns the correct event type enum', function () {
    ...
    expect($event->eventType())->toBe(WebhookEventType::BillingStatementLineItemDeleted);
});
...
it('exposes the full payload and metadata', function () {
    ...
    expect($event->payload['id'])->toBe('evt_cTMaFLRpkDJnFLKgnkdKsh1NWvReCHCS')
