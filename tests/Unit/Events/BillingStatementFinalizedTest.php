...
it('returns event data as a typed DTO via data()', function () {
    $payload = loadFixture('webhooks/billing_statement.finalized.json');
    $event = new BillingStatementFinalized($payload);
    ...
    expect($data)
        ...
        ->and($data->paymentIntent->id)->toBe('pi_iN1qyQ18iNHL9AyAuYayN1b1PJvdnH97')
...
it('returns the correct event type enum', function () {
    ...
    expect($event->eventType())->toBe(WebhookEventType::BillingStatementFinalized);
});
...
it('exposes the full payload and metadata', function () {
    ...
    expect($event->payload['id'])->toBe('evt_YKgzJ5rKNje5SxfkvAEqvfRb48vqosYu')
