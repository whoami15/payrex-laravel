...
it('returns event data as a typed DTO via data()', function () {
    ...
    expect($data)
        ...
        ->and($data->id)->toBe('bstm_vrNSubLZQMfPiGc2EpDiCF4HTu4aiPwV')
        ->and($data->amount)->toBe(50000)
        ->and($data->currency)->toBe('PHP')
        ->and($data->customerId)->toBe('cus_fE64t8BJ5e7PvhZNWG6DCjbR2ASCp9iw')
        ->and($data->status)->toBe(BillingStatementStatus::Draft)
        ->and($data->description)->toBe('Test billing statement')
        ->and($data->billingStatementUrl)->toBe('https://bill.payrexhq.com/b/test_bstm_vrNSubLZQMfPiGc2EpDiCF4HTu4aiPwV_secret_RJFPaYbnkcRCCufpQF2p17jzcEn9BxHnsFjeh5sMVwW1vALQE8')
        ...
        ->and($data->customer->id)->toBe('cus_fE64t8BJ5e7PvhZNWG6DCjbR2ASCp9iw')
...
it('exposes the full payload and metadata', function () {
    ...
    expect($event->payload['id'])->toBe('evt_zF6MRvjTtYM2MyHEgDJCUdG9LdhX5S6c')
