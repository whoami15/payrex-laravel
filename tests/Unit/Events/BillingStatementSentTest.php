...
it('returns event data as a typed DTO via data()', function () {
    $payload = loadFixture('webhooks/billing_statement.sent.json');
    $event = new BillingStatementSent($payload);
...
it('returns the correct event type enum', function () {
    ...
    expect($event->eventType())->toBe(WebhookEventType::BillingStatementSent);
});
...
it('exposes the full payload and metadata', function () {
    ...
    expect($event->payload['id'])->toBe('evt_FueyrXtA9rhMZUBqg3JvJAUDD9CqpkC7')
