...
it('returns event data as a typed DTO via data()', function () {
    ...
    expect($data)
        ...
        ->and($data->id)->toBe('bstm_EzafBdpSQB1FHBWj3U4ZPZkVTfi2GfkJ')
        ->and($data->amount)->toBe(50000)
        ->and($data->currency)->toBe('PHP')
        ->and($data->customerId)->toBe('cus_pwCPewCNMsmZu98woAtP5isdnL164Q7W')
        ...
        ->and($data->customer->id)->toBe('cus_pwCPewCNMsmZu98woAtP5isdnL164Q7W')
        ->and($data->paymentIntent)->toBeInstanceOf(PaymentIntent::class)
        ->and($data->paymentIntent->id)->toBe('pi_VTcMqhcsEFiAQQ6vBajbsyHXqXa8f2e2')
...
it('exposes the full payload and metadata', function () {
    ...
    expect($event->payload['id'])->toBe('evt_ND27paq3da3wwFRSJitDu3uvtd6tZjZb')
