...
it('returns the configured default currency', function () {
    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com', currency: 'USD');

    expect($client->defaultCurrency())->toBe('USD');
});

it('defaults currency to PHP', function () {
    $client = new PayrexClient(secretKey: 'sk_test_123', baseUrl: 'https://api.payrexhq.com');

    expect($client->defaultCurrency())->toBe('PHP');
});
