...
it('defaults resource to empty string when missing', function () {
    $obj = new PayrexObject(['id' => 'obj_123']);

    expect($obj->resource)->toBe('');
});

it('defaults livemode to false when missing', function () {
    $obj = new PayrexObject(['id' => 'obj_123', 'resource' => 'test']);

    expect($obj->livemode)->toBeFalse()
