...
it('hydrates all properties from fixture', function () {
    ...
    expect($session->id)->toBe('cs_xxxxx')
        ->and($session->resource)->toBe('checkout_session')
        ->and($session->amount)->toBe(100000)
        ->and($session->clientSecret)->toBe('cs_xxxxx_secret_xxxxx')
        ...
        ->and($session->url)->toBe('https://checkout.payrexhq.com/c/cs_xxxxx_secret_xxxxx')
    ...
    expect($paymentIntent->id)->toBe('pi_xxxxx')
